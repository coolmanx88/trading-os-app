...
function renderDashboardPending(){
  ...
  if(!card){
    ...
    card.dataset.pendingReviewCardV24='1';
    card.className='pending-review-dashboard';
...
function markPendingRows(){
  ...
  document.querySelectorAll('tr').forEach(row=>{
    ...
    if(!cells.length||row.querySelector('.pending-review-badge'))return;
    const target=cells[cells.length-1];
    const badge=document.createElement('span');badge.className='pending-review-badge';badge.textContent='بانتظار المراجعة';badge.style.marginInlineStart='6px';target.appendChild(badge);
  });
}
