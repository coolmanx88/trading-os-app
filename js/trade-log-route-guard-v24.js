...
function loadState(){
  try{return JSON.parse(localStorage.getItem(STATE_KEY)||'{}')}catch{return{trades:[]}}
}
function tradeById(id){return (loadState().trades||[]).find(t=>t.id===id)||null}
function tradeIdFromNode(node){
  if(!node?.closest)return null;
  const row=node.closest('tr');
  const scope=row||node.closest('[data-trade-id],.trade-card,.card,button,a')||node;
  return (scope.textContent||'').match(/TR-[A-Za-z0-9-]+/)?.[0]||null;
}
function activeHashId(){
  const m=String(location.hash||'').match(/^#active\?id=([^&]+)/i);
  if(!m)return null;
...
function neutralizeClosedTradeLinks(){
  ...
  document.querySelectorAll('tr').forEach(row=>{
    const id=(row.textContent||'').match(/TR-[A-Za-z0-9-]+/)?.[0];
    const t=id?closed.get(id):null;if(!t)return;
    row.dataset.closedTradeRow='1';
    row.querySelectorAll('button[onclick*="#active?id="],a[onclick*="#active?id="]').forEach(btn=>{
      if(btn.dataset.closedRouteFixed==='1')return;
      btn.dataset.closedRouteFixed='1';
      btn.removeAttribute('onclick');
      btn.onclick=e=>{e.preventDefault();e.stopPropagation();openClosedTrade(t)};
    });
  });
}
...
window.addEventListener('hashchange',()=>{
  guardActiveHash();
  setTimeout(neutralizeClosedTradeLinks,0);
},true);
window.addEventListener('load',()=>{
  guardActiveHash();
  neutralizeClosedTradeLinks();
});
